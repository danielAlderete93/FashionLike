...
public class ReactionTypeMapperPersistence implements MapperPersistence<ReactionTypeEntity, ReactionType> {
    ...
    public ReactionType toDomain(ReactionTypeEntity entity) {
        ...
        return ReactionType.builder().id(entity.getId()).name(entity.getName()).emoji(entity.getEmoji()).build();
    }

    @Override
    public ReactionTypeEntity toEntity(ReactionType domain) {
        return ReactionTypeEntity.builder().id(domain.getId()).name(domain.getName()).emoji(domain.getEmoji()).build();
    }
}
